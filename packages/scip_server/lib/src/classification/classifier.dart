/// Symbol classification for layer and feature detection.
///
/// Provides heuristics to classify symbols into architectural layers
/// (UI, Service, Data, Model) and detect feature groupings.
library;

import '../index/scip_index.dart';
import 'types.dart';

export 'types.dart' show SymbolLayer, SymbolClassification;

/// Classifies symbols into architectural layers.
class LayerClassifier {
  LayerClassifier(this.index) {
...
/// Detects feature/module groupings for symbols.
class FeatureDetector {
  FeatureDetector(this.index);
  ...
  /// - `features/<name>/`
  /// - `modules/<name>/`
  /// - `lib/<name>/` (if not a common directory)
  ...
  /// - `UserProfilePage` → `user` or `userProfile`
  ...
  ///
  /// Symbols that frequently call each other are likely part of the same feature.
...
/// Combined classifier that uses both layer and feature detection.
class SymbolClassifierService {
  SymbolClassifierService(ScipIndex index)
      : _layerClassifier = LayerClassifier(index),
        _featureDetector = FeatureDetector(index),
        _index = index;

  final LayerClassifier _layerClassifier;

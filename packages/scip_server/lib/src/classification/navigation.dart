/// Navigation detection for storyboard generation.
///
/// Detects navigation patterns in Flutter code to build
...
import 'navigation_chain.dart';
import 'types.dart';

export 'types.dart' show NavigationEdge, RouterType, NavigationGraph;

/// Extension to add feature grouping to NavigationGraph.
extension NavigationGraphFeatureGrouping on NavigationGraph {
  /// Get screens grouped by feature.
  Map<String, List<SymbolInfo>> groupByFeature(FeatureDetector detector) {
    final grouped = <String, List<SymbolInfo>>{};
    for (final screen in screens) {
      final feature = detector.detectFeature(screen) ?? 'main';
      grouped.putIfAbsent(feature, () => []).add(screen);
    }
    return grouped;
  }
}

/// Detects navigation patterns and builds storyboard graphs.
class NavigationDetector {
  NavigationDetector(this.index);

  final ScipIndex index;

  /// Chain extractor for detailed navigation context (lazy initialized).
  NavigationChainExtractor? _chainExtractor;

  /// Get or create the chain extractor with SCIP type info.
  NavigationChainExtractor get chainExtractor {
    return _chainExtractor ??= NavigationChainExtractor(
  ...
  /// Legacy method - use findPages() instead.
  @Deprecated('Use findPages() for accurate page detection')
  List<SymbolInfo> findScreens() {
    final screens = <SymbolInfo>[];
  ...
  /// Generate Mermaid flowchart from navigation graph.
  String generateMermaid(
    ...
    final uniqueEdges = graph.edges.toSet().toList();

    // Track written edges to avoid duplicates in output
    final writtenEdges = <String>{};

    // Track which nodes need labels (dynamic routes)
    final declaredNodes = <String>{};

    if (featureDetector != null) {
      // Group by feature
      final grouped = graph.groupByFeature(featureDetector);

      for (final entry in grouped.entries) {
        final feature = entry.key;
        final screens = entry.value;

        buffer.writeln('    subgraph ${_sanitizeId(feature)} [$feature]');

        // Add edges within this feature
        for (final screen in screens) {
          final outEdges =
              uniqueEdges.where((e) => e.fromScreen == screen.name);
          for (final edge in outEdges) {
            final label = edge.label ?? edge.trigger ?? '';
            final edgeKey = '${edge.fromScreen}|${edge.toScreen}|$label';
            if (writtenEdges.contains(edgeKey)) continue;
            writtenEdges.add(edgeKey);

            // Declare nodes with labels if they're dynamic routes
            _declareNodeIfNeeded(buffer, edge.toScreen, declaredNodes,
                indent: '        ');

            if (label.isNotEmpty) {
              buffer.writeln(
                  '        ${_sanitizeId(edge.fromScreen)} -->|"$label"| ${_sanitizeId(edge.toScreen)}');
            } else {
              buffer.writeln(
                  '        ${_sanitizeId(edge.fromScreen)} --> ${_sanitizeId(edge.toScreen)}');
            }
          }
        }
      ...
      // No grouping, just add all edges
  ...
  /// Declare a Mermaid node with a label if it's a dynamic route.
  void _declareNodeIfNeeded(
    StringBuffer buffer,
    String screenName,
    Set<String> declaredNodes, {
    required String indent,
  }) {
    // Check if this is a dynamic route (contains $ or / or {)
    if (!_isDynamicRoute(screenName)) return;

    final nodeId = _sanitizeId(screenName);
    if (declaredNodes.contains(nodeId)) return;
    declaredNodes.add(nodeId);

    // Declare the node with a readable label
    buffer.writeln('$indent$nodeId["$screenName"]');
  }

  /// Check if a screen name looks like a dynamic route.
  bool _isDynamicRoute(String name) {
  ...
  /// Generate ASCII art diagram from navigation graph.
  String generateAscii(NavigationGraph graph) {
    final buffer = StringBuffer();
    ...
    final allTargets = graph.edges.map((e) => e.toScreen).toSet();
    final roots = graph.screens
        .map((s) => s.name)
        .where((s) => !allTargets.contains(s))
        .toList();

    if (roots.isEmpty && graph.screens.isNotEmpty) {
      roots.add(graph.entryScreen ?? graph.screens.first.name);
    }
...
///
/// Similar to widget_definition_visitor.dart's `_SimpleScaffoldChecker`.
/// Uses a depth limit to avoid infinite recursion and focus on direct usage.
